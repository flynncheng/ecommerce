...
import ProductActionSelect from "./ProductActionSelect";
...
export default function ProductAction({
  ...
  const addColorOptionImage = (option: HttpTypes.StoreProductOption) => {
    if (option.title === "Color" && option.values) {
      option.values.forEach((optionValue) => {
        optionValue.image_url = product.images.find((image) =>
          image.url.includes(optionValue.value.toLowerCase()),
        ).url;
      });
    }
  };
  ...
  return (
    <div className="mt-3">
      ...
      {inStock && selectedPrice && (
        <p className="mt-5 px-2 text-sm text-gray-500">
          Due to high demand, your product will be shipped in 3 to 4 days.
        </p>
      )}
    </div>
  );
